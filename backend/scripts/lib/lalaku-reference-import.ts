...
function normalizeTerritoryLabel(s: string): string {
  const t = s.trim().replace(/\s+/g, " ");
  return t.toUpperCase() === t ? t : t.toUpperCase();
}

function findTerritoryNodesByNameKey(forest: TerritoryNode[], nameKey: string): TerritoryNode[] {
  const hits: TerritoryNode[] = [];
  const walk = (list: TerritoryNode[]) => {
    for (const n of list) {
      if (normKey(n.name) === nameKey) hits.push(n);
      walk(n.children);
    }
  };
  walk(forest);
...
export function mergeCitiesIntoTerritoryForest(
  ...
  for (const row of rows) {
    ...
    const dup = regionNode.children.some((ch) => {
      if (code && ch.code && normKey(ch.code) === normKey(code)) return true;
      return normKey(ch.name) === cKey;
    });
    if (dup) {
      skipped_duplicate++;
      continue;
