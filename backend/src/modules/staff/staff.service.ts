...
function entitlementsSnapshotFromAgentUser(u: {
  ...
  const fromJsonPts =
    Array.isArray(jPt) && jPt.every((x) => typeof x === "string")
      ? jPt.map((s) => String(s).trim()).filter(Boolean)
      : [];
  const colPts = parsePriceTypesJson(u.agent_price_types);
  const single = u.price_type?.trim();
  const price_types = [...new Set([...fromJsonPts, ...colPts, ...(single ? [single] : [])])];
...
export async function bulkPatchAgents(
  ...
  switch (input.action) {
    case "set_agent_entitlements": {
      ...
      await validateAgentEntitlements(tenantId, input.agent_entitlements);
      for (const id of ids) {
        await applyAgentPatchInDb(tenantId, id, { agent_entitlements: input.agent_entitlements }, actorUserId);
      }
    ...
    case "patch_product_list": {
      ...
      for (const u of users) {
        let snap = entitlementsSnapshotFromAgentUser(u);
        if (pids.length && input.category_id != null) {
          snap = mergeAgentProductRulesPatch(snap, input.mode, input.category_id, pids);
        }
        if (pts.length) {
          snap = mergeEntitlementPriceTypes(snap, input.mode, pts);
        }
        const ent = snapToAgentEntitlements(snap);
        await validateAgentEntitlements(tenantId, ent);
