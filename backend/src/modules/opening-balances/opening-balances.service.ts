    ...
    if (bal) {
      ...
      await tx.clientBalanceMovement.create({
        data: {
          client_balance_id: bal.id,
          delta: reverseDelta,
          note: `Начальный баланс #${entry.id} удалён`,
          user_id: uid
        }
      });
    }

    await tx.clientOpeningBalanceEntry.delete({ where: { id: entryId } });
  });

  await appendClientAuditLog(tenantId, clientId, actorUserId, "client.opening_balance.delete", {
    entry_id: entryId
  });
